...
int neul_bc95_hw_detect(void)
{
    ...
    ret = neul_dev.ops->dev_read(neul_dev.rbuf, NEUL_MANUFACT_LEN, 0, 2000);
    if (ret <= 0)
    {
...
int neul_bc95_active_network(void)
{
    ...
    if (ret < 0)
    {
        return -1; //write data to bc95 failed
    }
    ret = neul_dev.ops->dev_read(neul_dev.rbuf, 32, 0, 200);
    if (ret <= 0)
    {
        return -1; //read bc95 read set return value info failed
    }
    str = strstr(neul_dev.rbuf,"+CGSN:");
    if (!str)
    {
...
int uart_data_read(char *buf, int maxrlen, int mode, int timeout)
{
    ...
    {
        //block mode
        retlen = los_dev_uart_read(LOS_USART3, buf, maxrlen, timeout);
    }
    else
    {
        //none block mode
        retlen = los_dev_uart_read(LOS_USART3, buf, maxrlen, 0);
    }
...
int uart_data_write(char *buf, int writelen, int mode)
{
    ...
    los_dev_uart_write(LOS_USART3, buf, writelen, 500);
    return 0;
}
