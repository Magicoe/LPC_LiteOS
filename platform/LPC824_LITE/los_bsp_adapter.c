...
/*
    if g_use_ram_vect == 1, we should use sct file LPC82x-LiteOS.sct
...
unsigned int osTickStart(void)
{
    ...
    /* 
      ...
      Note: here can be replaced by some function , for example in LPC82x bsp
...
{
    /*
        enable irq , for example in LPC82x bsp you can use
        NVIC_EnableIRQ((IRQn_Type)irqnum);
    */
    nvicSetIRQ(irqnum);
    /*
        set irq priority , for example in LPC82x bsp you can use
...
{
    /*
        disable irq, for example in LPC82x bsp you can use
