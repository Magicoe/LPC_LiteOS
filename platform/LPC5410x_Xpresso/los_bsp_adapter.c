...
volatile unsigned int sys_clk_freq = 16000000;   /* MGN, should be changable based on different system */
...
/*
    if g_use_ram_vect == 1, we should use sct file LPC5410x-LiteOS.sct
...
unsigned int osTickStart(void)
{
    ...
    /* 
      ...
      Note: here can be replaced by some function , for example in LPC5410x bsp
...
{
    /*
        enable irq , for example in LPC5410x bsp you can use
        NVIC_EnableIRQ((IRQn_Type)irqnum);
    */
    nvicSetIRQ(irqnum);
    /*
        set irq priority , for example in LPC5410x bsp you can use
...
{
    /*
        disable irq, for example in LPC5410x bsp you can use 
