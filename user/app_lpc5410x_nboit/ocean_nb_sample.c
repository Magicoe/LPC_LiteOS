...
#define LOS_DEV_UART_BUF_SIZE 1024
volatile uint8_t los_uart_buf[LOS_DEV_UART_BUF_SIZE] = {0};

char nbget_show_data(int idx)
{
	if (idx < 0  || idx > 38)
	{
		return '0';
...
void oceancon_nb_task(void * pvParameters)
{
    ...
    GPIO->DIR[1] |= 1U << 6;
    GPIO->B[1][6] = 0;
    IOCON_PinMuxSet(IOCON, 1, 6, (IOCON_MODE_PULLUP | IOCON_FUNC0 | IOCON_GPIO_MODE | IOCON_DIGITAL_EN | IOCON_INPFILT_OFF));
...
void oceancon_nb_sampletask(void * pvParameters)
{
    ...
    ret = los_dev_uart_init(LOS_USART3, 9600, los_uart_buf, LOS_DEV_UART_BUF_SIZE);
    if (ret != 0)
    {
        while(1);
    }
